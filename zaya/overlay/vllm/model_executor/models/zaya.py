...
class ZayaRouter(nn.Module):
    def __init__(
        ...
        # ---- Layers ----
        # The router is accuracy-sensitive (top-1 routing) and must stay in
        # high precision. These Linears are built without a prefix, so the
        # quant-config ignore list can't match them by name — pass
        # quant_config=None so they are never quantized regardless.
        ...
        # quant_config=None: router stays bf16 (see down_proj note above).
        self.router_mlp = nn.Sequential(
            ReplicatedLinear(
                D, D, bias=True, quant_config=None, return_bias=False
            ),
            self.non_linearity,
            ReplicatedLinear(
                D, D, bias=True, quant_config=None, return_bias=False
            ...
            self.non_linearity,
            ReplicatedLinear(
                D, E, bias=False, quant_config=None, return_bias=False
            ),
        )
...
class ZayaForCausalLM(nn.Module, HasInnerState, IsHybrid, SupportsEagle3):
    ...
    # num_accepted_tokens). Needed to make CCA spec decode bit-lossless. Default
    # OFF so the normal serving path (align/none) is unchanged; the CCA 'all'-mode
    # forward is still being brought up (see project-spec-decode-blocker memory).
    supports_mamba_prefix_caching: bool = (
        os.environ.get("ZAYA_SPEC_ALL", "0") == "1"
    )
    ...
    def get_mamba_state_shape_from_config(
        ...
        # Must match the layer instance's get_state_shape exactly: it drives the
        # mamba/attention page-size padding. The CCA conv window is the combined
        # receptive field of both conv layers (ZayaCCA.total_padding =
        # (cca_time0-1)+(cca_time1-1)), widened by num_spec for the spec-decode
        # rollback window.
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        # Prefix caching uses the "align" mamba-cache mode (the proven path for
        # hybrid models; "all" mode inflates the KV block size and overflows the
        # gfx1201 64KB LDS in chunked_prefill_paged_decode). The CCA conv +
        # temporal state is copied per-block via get_mamba_state_copy_func.
        # Requires chunked prefill (on by default). Validate accuracy.
            ...
            if "local_experts" in chkpt_weight_name:
                ...
                if parts[-2] == "linear_fc1":
                    ...
                    fused_moe_module.weight_loader(
                        param, gate, chkpt_weight_name, "w1", expert_id
                    )
                    fused_moe_module.weight_loader(
                        param, up, chkpt_weight_name, "w3", expert_id
                    )
