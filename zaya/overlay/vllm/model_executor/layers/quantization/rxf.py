...
class RXFConfig(QuantizationConfig):
    def __init__(self, group_size, ignore, full_config, codebook=None,
                 ...
                 fp16_experts=None, act_dtype="fp16"):
        super().__init__()
        ...
        # Optional per-group Hadamard rotation ("hadamard32" or None). MANDATORY
        # gate: weights are stored pre-rotated, so the kernel MUST apply the
        # matching activation rotation or the matmul is silently wrong. A plain
        # (un-rotated) checkpoint leaves this None and the kernel stays identity.
    ...
    def from_config(cls, config: dict[str, Any]) -> "RXFConfig":
        ...
        # Per-group Hadamard rotation gate (mandatory if the checkpoint was
        # quantized with --rotate; absent/None on plain checkpoints).
        rotation = weights.get("rotation") or config.get("rotation")
        if rotation is not None and rotation != "hadamard32":
            raise ValueError(
                f"rxf unknown rotation {rotation!r}; expected "
                f"'hadamard32' or null")
...
class RXFLinearMethod(LinearMethodBase):
    ...
    def process_weights_after_loading(self, layer) -> None:
        ...
        # self.quant_config values, so the two paths can never diverge.
...
class RXFFusedMoEMethod(FusedMoEMethodBase):
    ...
    def apply_monolithic(self, layer, x, router_logits, input_ids=None):
        ...
        return self._experts.apply(
            ...
            # router_logits (ZAYA MoD/EDA) select the right experts.
