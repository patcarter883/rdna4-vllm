...
class RoutedExperts(PluggableLayer):
    ...
    def _het_moe_shard(self, axis_total: int, tp_rank: int, tp_size: int):
        ...
        per-partition sizing in ``fused_moe/config.py``. ``axis_total`` is
        ``loaded_weight.shape[shard_dim]`` (axis units: packed/grouped/plain)."""
        ...
        return het_axis_shard(self.moe_config.intermediate_size, HET_ALIGN_FLOOR,
    ...
    def _load_combined_w13_weight_scale(
        ...
        if het_active(self.moe_config.moe_parallel_config.tp_size):
    ...
    def _load_w13(
        ...
        if not load_full and loaded_weight.ndim > 0:
            ...
            # Heterogeneous TP: proportional offset/size along the intermediate axis
            # (no-op even split unless VLLM_TP_CU_WEIGHTS is set).
            start_offset, loaded_per_rank = self._het_moe_shard(
