...
def _cast_kv_tile(data, Q, tensor_scale, KV_QUANT_MODE: tl.constexpr):
    ...
    """
    # All modes plain-cast here. FP8 per-tensor (mode 1) scales are folded into
    # score_scale / acc by the caller (FOLD_KV_SCALE), and per-token-head modes
    # (2, 3) apply their scales on S/P -- so the inner loop never does a
    # per-element fp32 dequant multiply. For fp8 Q this casts K to fp8 (native
    # WMMA); for bf16 Q it casts fp8 K -> bf16 (one cast, no multiply).
...
def kernel_unified_attention(
    ...
    # FP8 per-tensor KV: fold the scalar k/v scales into the QK^T score and the
    # P·V accumulator instead of dequantizing every K/V element in the inner
    # loop. This covers the common bf16-Q case (Q_IS_FP8 == False) that the old
    # code missed -- it took the per-element fp32 dequant path for no matmul
    # speedup. Mathematically exact: scales are scalars, S = scale*k_scale*QK^T
    # and acc = v_scale*(P·V).
    FOLD_KV_SCALE: tl.constexpr = KV_QUANT_MODE == 1
    ...
    score_scale = scale
    value_scale = 1.0
    # FP8 per-tensor KV: fold scalar k/v scales here rather than per-element in
    # _cast_kv_tile. Works for bf16 Q and fp8 Q; only fp8 Q also descales Q.
    if FOLD_KV_SCALE:
        score_scale = score_scale * tl.load(k_scale)
        value_scale = tl.load(v_scale)
    if USE_FP8_Q_DESCALE:
        score_scale = score_scale * tl.load(q_scale)
    ...
    for j in range(loop_lo, loop_hi):
        ...
        # TiDAR (Route B) bidirectional query-query region: when a qq_bias is
        # supplied, the bias tensor alone is the allow/deny truth for keys that
        # correspond to query rows (the "new-token" region), so the causal
        # seq_mask must NOT additionally restrict there -- otherwise -inf+finite
        # stays -inf and a query can never attend a query-key to its RIGHT (the
        # replica block's bidirectional siblings).  PREFIX keys (key_rel_pos < 0)
        # stay strictly causal.  The is_qq predicate is identical to the one
        # load_qq_bias_tile uses to decide which keys carry a bias (key_rel_pos
        # in [0, qq_bias_stride_0)), so the gate and the bias-load agree exactly.
        # For existing tree-attn callers the qq_bias is itself causal-or-stricter,
        # so OR-ing the causal mask back on then adding their (already-causal)
        # bias is a no-op -- the Part D regression case asserts this.  When
        # USE_QQ_BIAS is False this whole block is dead-code-eliminated, leaving
        # the stock path byte-identical.
        if USE_QQ_BIAS:
            key_rel_pos = seq_offset - context_len
            is_qq = (key_rel_pos >= 0) & (key_rel_pos < qq_bias_stride_0)
            seq_mask = seq_mask | is_qq[None, :]
...
def unified_attention(
    ...
    # Max query_len that still counts as "decode" for 3D flash-decode
    # eligibility. 1 for plain decode; num_spec+1 for spec-decode verify.
    decode_query_len_threshold: int = 1,
    alibi_slopes=None,
    output_scale=None,
    qq_bias=None,
    ...
    use_td: bool = False,
    # Offline-tuned launch knobs (resolved at vLLM launch from the per-device
    # attention tuning config; threaded through TritonAttentionMetadata).
    # All default to None == today's behavior: decode tile from _get_tile_size
    # and Triton's own num_warps/num_stages defaults. Set only via the tuning
    # config or the offline tuner.
    tile_size_decode: int | None = None,
    # Offline-tuned tile for the 2D-grid launch (genuine prefill AND the
    # high-batch decode failover that routes off the 3D path). Overrides
    # TILE_SIZE_PREFILL (default 32 from _get_tile_size). None == today's
    # behavior. The caller (runtime selector) decides WHEN to apply it; the
    # kernel just honors the override if given.
    tile_size_2d: int | None = None,
    num_warps: int | None = None,
    num_stages: int | None = None,
    # ROCm/HIP-only launch knob (amdgpu-waves-per-eu fn attr). None == Triton's
    # HIPOptions default (0). Ignored on CUDA (the CUDA backend has no such
    # field) — only set it on ROCm via the tuner / tuned config.
    ...
    TILE_SIZE_DECODE = (
        tile_size_decode
        if tile_size_decode is not None
        else _get_tile_size(
    ...
    #
    # Hard row-capacity bound: the 3D store indexes the segm_* buffers by
    # global query-token row and has NO in-kernel row mask, so safety rests
    # entirely on this gate. Bound against the buffer's ACTUAL first dim
    # (min with seq_threshold_3D) rather than trusting the two to match, so a
    # builder/gate size mismatch can never OOB — an over-capacity batch routes
    # to the 2D path instead.
    segm_rows = softmax_segm_output.shape[0] if softmax_segm_output is not None else 0
    use_3d = not (
        ...
        or max_seqlen_q > decode_query_len_threshold
        # Capacity: the segm_* buffers are indexed by global query-token row,
        # so the TOTAL query rows must fit, not just num_seqs. For plain decode
        # q.shape[0] == num_seqs, so this is identical to the old gate; for
        # spec-verify it counts the extra num_spec rows per sequence.
        or q.shape[0] > min(seq_threshold_3D, segm_rows)
    ...
    # Offline-tuned Triton launch knobs. When None (default) they are omitted
    # entirely so Triton applies its own heuristics — byte-identical to the
    # pre-tuning launch.
    _launch_kw: dict[str, Any] = {}
    if num_warps is not None:
        _launch_kw["num_warps"] = num_warps
    if num_stages is not None:
        _launch_kw["num_stages"] = num_stages
    if waves_per_eu is not None:
        _launch_kw["waves_per_eu"] = waves_per_eu
