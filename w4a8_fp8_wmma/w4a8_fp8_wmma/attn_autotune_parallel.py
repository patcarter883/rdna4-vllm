...
Gate: VLLM_ATTN_AUTOTUNE_PARALLEL (default "1" = ON; set "0" to disable). Workers:
VLLM_ATTN_AUTOTUNE_PARALLEL_WORKERS (default 6 — measured sweet spot; more contends).
Validated GPU1: 2.77x on the cold attention autotune. On a WARM cache the workers'
warmup() calls cache-hit, so it adds only the small pool-spawn overhead, no benefit.
"""
...
def prewarm(**kwargs) -> None:
    ...
    workers = int(os.environ.get("VLLM_ATTN_AUTOTUNE_PARALLEL_WORKERS",
                                 min(6, max(1, (os.cpu_count() or 2) - 2))))
...
def install() -> None:
    ...
    DEFAULT ON — set VLLM_ATTN_AUTOTUNE_PARALLEL=0 to disable."""
    if os.environ.get("VLLM_ATTN_AUTOTUNE_PARALLEL", "1") != "1":
        return
    from vllm.v1.attention.ops import attn_autotune as A
