"""Validate the v11 K%512 relaxation (512-k tail).

Two checks:
  (A) KERNEL: call torch.ops.w4a8_fp8_wmma.mmq_fp8_gemm(..., ver=11) directly for K
  ...
  (B) DISPATCH: through the real adapter, M=1/2 with K=8704 must now pick v11 (not v10).
...
def kernel_v11(M, K, N, G, asym):
    """Direct ver=11 op call. Returns (rel_err, ok)."""
    dev = "cuda"
    wp, sc, zeros = make_weights(N, K, G, asym, dev)
    x = torch.randn(M, K, dtype=torch.float16, device=dev) * 0.5
    zp_in = zeros if zeros is not None else torch.empty(0, dtype=torch.int32, device=dev)
    out = torch.ops.w4a8_fp8_wmma.mmq_fp8_gemm(x, wp, sc, zp_in, 11).float()
...
def main():
    ...
    allok = True
    print("(A) KERNEL ver=11 on K%512-not-%1024 (down_proj K=8704 + neighbours):")
