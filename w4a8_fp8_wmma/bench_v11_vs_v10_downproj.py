"""Payoff check: v11 (GEMV) vs v10 (WMMA) for the down_proj decode shape that the
K%512 fix unlocks. N=5120, K=8704, gs=32, asym (AWQ), M=1/2/4. Reports per-call us
and effective weight-read GB/s (weights = N*K/2 bytes int4 + scales/zeros)."""
import torch, time
...
def bench(ver, x, wp, sc, zp, iters=200):
    f = lambda: torch.ops.w4a8_fp8_wmma.mmq_fp8_gemm(x, wp, sc, zp, ver)
    for _ in range(20):
        f()
...
def main():
    ...
    for M in (1, 2, 4):
        ...
        t10 = bench(10, x, wp, sc, zp)
        t11 = bench(11, x, wp, sc, zp)
